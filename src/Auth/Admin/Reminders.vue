...
const saveReminder = async () => {
    try {
        ...
        const month = (form.value.date.getMonth() + 1).toString().padStart(2, '0')
        const day = form.value.date.getDate().toString().padStart(2, '0')

        const dateStr = `${year}-${month}-${day}`
        const localDateTime = `${dateStr} ${formattedHour}:00:00`
